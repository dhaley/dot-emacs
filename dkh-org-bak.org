...
;;  (add-hook 'org-mode-hook 'turn-on-flyspell 'append) ;; took out dkh
...
This can happen if you are in the body of a heading and globally fold the org-file with
S-TAB=

I find invisible edits (and undo's)
hard to deal with so now I can't edit invisible text.
=C-c C-r= (org-reveal)
will display where the point is if it is buried in invisible text
