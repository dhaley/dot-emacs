...
Keys are in kbd format."
    ...
    (defun buffer-projectile (change-buffer-fun)
      (let ((proj-mode (projectile-project-buffer-p (current-buffer) default-directory))
            (next-mode nil))
        (while (not (eq next-mode proj-mode))
          (funcall change-buffer-fun)
          (setq next-mode (projectile-project-buffer-p (current-buffer) default-directory)))))
    ...
    (defun projectile-switch-to-next-buffer (&optional window)
...
found."
    ...
    (defun projectile-switch-to-prev-buffer (&optional window bury-or-kill)
